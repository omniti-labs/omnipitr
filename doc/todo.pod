...
=item * report missing statedir on start, and to logs, and not to stderr (or to
both, but add to logs)

=item * make omnipitr-restore don't care about walarchive writability if -r
option is not given
