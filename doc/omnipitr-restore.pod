...
This delay does not apply to initial recovery (getting system from backup till
"Minimum recovery ending location"). Reason for this is simple - before reaching
minimum recovery ending location database can be in nonconsistent state. So
there is no point in stalling till we get to point where database is consistent.
