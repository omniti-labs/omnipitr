...
2 signals are supported:

=over

=item * SIGUSR1

makes the finish I<smart>
...
=head3 Minimal setup, but with backing up segments to remote server:
    ...
    restore_command='/.../omnipitr-restore -l /var/log/omnipitr/restore.log -s /mnt/wal_restore/ -h "/.../omnipitr-archive -l /var/log/omnipitr/archive.log -dr bzip2%rsync://backup/postgres/xlogs/" %f %p'
